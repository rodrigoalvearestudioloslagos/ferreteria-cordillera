...
const productos = [
  ...
  },
  {
    ...
    categoria: "Herramientas Eléctricas",
    emoji: "⚙️",
    specs: ["850W", "Disco 115 mm", "Protector ajustable", "Ideal para corte y desbaste"],
  },
  {
    id: 3,
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <header className="sticky top-0 z-50 bg-slate-950 text-white shadow-xl">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between gap-6">
          <div>
            ...
            <p className="text-xs text-slate-400">Todo para construir, reparar y mejorar</p>
          </div>
      ...
      {/* HERO */}
      <section id="inicio" className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 text-white">
        <div className="max-w-7xl mx-auto px-6 py-24 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="mt-8 text-xl text-slate-300">
              Herramientas, materiales, pinturas, electricidad y productos para construcción,
              con cotización directa por WhatsApp.
            </p>

            <div className="mt-10 flex gap-4 flex-wrap">
              <a href="#productos" className="bg-orange-500 px-8 py-4 rounded-xl font-black hover:bg-orange-600">
                Ver productos
              </a>
              <a href="#contacto" className="border border-white px-8 py-4 rounded-xl font-black hover:bg-white hover:text-slate-950">
          ...
          <div className="bg-white/10 backdrop-blur rounded-[2rem] p-8 shadow-2xl border border-white/10">
            <div className="grid grid-cols-2 gap-4">
              {productos.slice(0, 4).map((p) => (
                <div key={p.id} className="bg-white text-slate-900 rounded-3xl p-5 shadow-xl">
                  <div className="text-5xl mb-4">{p.emoji}</div>
                  <p className="text-xs font-bold text-orange-500">{p.marca}</p>
                  <h3 className="font-black">{p.nombre}</h3>
                  <p className="font-black text-xl mt-2">${p.precio.toLocaleString("es-CL")}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="categorias" className="py-20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <h2 className="text-5xl font-black text-center mb-12">Compra por sección</h2>
          ...
          <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-6">
            {[
              ...
              <div key={item} className="bg-white rounded-3xl p-6 shadow-lg hover:-translate-y-2 transition cursor-pointer text-center font-black">
                {item}
              </div>
            ))}
          </div>
      ...
      <section id="productos" className="bg-white py-20">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-orange-500 font-black text-center">PRODUCTOS DESTACADOS</p>
          <h2 className="text-5xl font-black text-center mb-12">Ofertas y recomendados</h2>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {productos.map((producto) => (
              <div key={producto.id} className="rounded-3xl overflow-hidden bg-slate-50 shadow-xl border border-slate-100">
                <div className="h-56 bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center text-8xl">
                  {producto.emoji}
                </div>

                <div className="p-6">
                  <p className="text-xs font-black text-orange-500">{producto.marca}</p>
                  <h3 className="text-xl font-black mt-1">{producto.nombre}</h3>
                  <p className="text-sm text-slate-500 mt-1">{producto.categoria}</p>
      ...
      {/* BENEFICIOS */}
      <section id="beneficios" className="py-20 bg-slate-100">
        <div className="max-w-6xl mx-auto px-6">
          <h2 className="text-5xl font-black text-center mb-12">Una experiencia más avanzada</h2>

          <div className="grid md:grid-cols-3 gap-8">
            {[
              ["🛒", "Carrito de compras", "El cliente puede agregar productos y ver el total."],
              ["👤", "Inicio de sesión", "Simulación visual para mostrar una futura cuenta de cliente."],
              ["📦", "Detalle de producto", "Cada producto puede mostrar especificaciones técnicas."],
              ["📱", "Diseño responsive", "Funciona bien en celular, tablet y computador."],
              ["💬", "Cotización rápida", "Botón directo para contacto comercial."],
              ["⭐", "Imagen profesional", "Más confianza para cerrar la venta del sitio."],
      ...
      <section id="contacto" className="bg-slate-950 text-white py-20">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <h2 className="text-5xl font-black mt-4">Solicita tu cotización</h2>

          <div className="grid md:grid-cols-3 gap-8 mt-12">
            <div className="bg-slate-800 rounded-3xl p-8">📞 <h3 className="font-black mt-3">Teléfono</h3><p>+56 9 XXXX XXXX</p></div>
            <div className="bg-slate-800 rounded-3xl p-8">📧 <h3 className="font-black mt-3">Correo</h3><p>ventas@ferreteriacordillera.cl</p></div>
            <div className="bg-slate-800 rounded-3xl p-8">📍 <h3 className="font-black mt-3">Dirección</h3><p>Puerto Varas, Chile</p></div>
          </div>

          <a href="https://wa.me/569XXXXXXXX" className="inline-block mt-12 bg-green-500 px-10 py-4 rounded-xl font-black hover:bg-green-600">
      ...
      {productoActivo && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center px-6">
          <div className="bg-white rounded-3xl max-w-lg w-full p-8 shadow-2xl">
            <button onClick={() => setProductoActivo(null)} className="float-right text-2xl font-black">
              ×
            </button>

            <div className="text-8xl text-center">{productoActivo.emoji}</div>
            <p className="text-orange-500 font-black mt-4">{productoActivo.marca}</p>
      ...
      {loginOpen && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center px-6">
          <form onSubmit={iniciarSesion} className="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl">
            <button type="button" onClick={() => setLoginOpen(false)} className="float-right text-2xl font-black">
              ×
            </button>

            <h2 className="text-3xl font-black mb-2">Iniciar sesión</h2>
            <p className="text-slate-500 mb-6">Accede a tus cotizaciones y compras.</p>

            <input className="w-full border rounded-xl px-4 py-3 mb-4" placeholder="Correo electrónico" />
            <input className="w-full border rounded-xl px-4 py-3 mb-6" placeholder="Contraseña" type="password" />
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex justify-end">
          <div className="bg-white w-full max-w-md h-full p-8 shadow-2xl overflow-y-auto">
            <button onClick={() => setCartOpen(false)} className="float-right text-2xl font-black">
              ×
            </button>
            ...
            {carrito.length === 0 ? (
              ...
              <div className="space-y-4">
                {carrito.map((item, index) => (
                  <div key={`${item.id}-${index}`} className="flex justify-between bg-slate-100 rounded-2xl p-4">
                    <div>
                      <p className="font-black">{item.nombre}</p>
                      <p className="text-sm text-slate-500">{item.marca}</p>
                    </div>
                    <p className="font-black">${item.precio.toLocaleString("es-CL")}</p>
                  </div>
                ))}

                <div className="border-t pt-6 mt-6 flex justify-between text-2xl font-black">
                  <span>Total</span>
                  <span className="text-orange-500">${total.toLocaleString("es-CL")}</span>
                </div>
